...
<nav class="navbar navbar-expand navbar-light bg-light">
	...
	<ul class="navbar-nav ml-auto">
		<?php if(isset($_SESSION['loggedUser'])): ?>
			...
			<li class="nav-item">
				<a href="index.php" class="btn btn-warning m-1"><?php echo $_SESSION['loggedUser']->name ?></a>
			</li>
		<?php else: ?>
			<li class="nav-item">
			...
			<?php foreach($posts as  $post): ?>
				...
				<div class="card mb-3" >
					<div class="card-header">
						<h3><?php echo $post->title; ?> <small class="float-right"><?php if(isset($_SESSION['loggedUser']) && $post->user_id ==	 $_SESSION['loggedUser']->id): ?>
					...
					<div class="card-footer">
						<button class="btn btn-info btn-sm btn-float-right">
							<?php $date = date_create($post->created_at); echo date_format($date,"Y-m-d"); ?>
						</button>
						<button class="btn btn-warning btn-sm btn-float-right">
							<?php echo $user->getUserWhitId($post->user_id)->name ?>
						</button>
					</div>
				</div>
			<?php endforeach; ?>	
...
<div class="text-center"><h2>IDemoooooo GIT</h2></div>
